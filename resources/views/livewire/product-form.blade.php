<div class="max-w-4xl mx-auto p-4">
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-600">
            {{ $productId ? 'Update existing product details.' : 'Add a new item to your inventory with ease.' }}
        </p>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <div class="mt-1 flex items-center">
                    ...
                    <input
                        type="number"
                        step="0.01"
                        wire:model.defer="original_price"
                        id="original_price"
                ...
                @if($savings > 0)
                    <div class="p-3 bg-yellow-50 border rounded">
                        <p class="text-sm font-medium">
                            Savings:
                            <span class="font-semibold">${{ number_format($savings, 2) }}</span>
                        </p>
                        <p class="text-sm text-gray-600">
                            Discount:
                            <span class="font-semibold">{{ $discount_percentage }}%</span>
            ...
            <div class="md:col-span-2">
                ...
                <div class="mt-1 flex gap-2">
                    <input
                        type="text"
                        wire:model.defer="exterior_image"
                        id="exterior_image"
                        class="block w-full rounded border p-2"
            ...
            <div class="md:col-span-2">
                ...
                <div class="space-y-2 mt-2">
                    @foreach($interior_images as $index => $img)
                        <div class="flex items-center gap-2">
                            <input
                                type="text"
                                wire:model.defer="interior_images.{{ $index }}"
                                class="flex-1 rounded border p-2"
                                placeholder="https://example.com/interior.jpg"
                            >
                            <button
                                type="button"
                                wire:click="removeInteriorImage({{ $index }})"
                                class="p-2 rounded bg-red-50 border"
                            >
                                Remove
                            </button>
                        </div>
                ...
                <div class="mt-2">
                    <button
                        type="button"
                        wire:click="addInteriorImage"
                        class="px-3 py-2 rounded border bg-gray-50"
                    >
                        + Add Another Image
                    </button>
                </div>
                ...
                <div class="space-y-2 mt-2">
                    @foreach($benefits as $i => $b)
                        <div class="flex items-center gap-2">
                            <input
                                type="text"
                                wire:model.defer="benefits.{{ $i }}"
                                class="flex-1 rounded border p-2"
                                placeholder="e.g. Waterproof"
                            >
                            <button
                                type="button"
                                wire:click="removeBenefit({{ $i }})"
                                class="p-2 rounded bg-red-50 border"
                            >
                                Remove
                            </button>
                ...
                <div class="space-y-2 mt-2">
                    @foreach($ingredients as $i => $ing)
                        <div class="flex items-center gap-2">
                            <input
                                type="text"
                                wire:model.defer="ingredients.{{ $i }}"
                                class="flex-1 rounded border p-2"
                ...
                <div class="space-y-2 mt-2">
                    @foreach($warnings as $i => $w)
                        <div class="flex items-center gap-2">
                            <input
                                type="text"
                                wire:model.defer="warnings.{{ $i }}"
                                class="flex-1 rounded border p-2"
                                placeholder="e.g. Keep away from children"
                            >
                            <button
                                type="button"
                                wire:click="removeWarning({{ $i }})"
                                class="p-2 rounded bg-red-50 border"
                            >
                                Remove
                            </button>
